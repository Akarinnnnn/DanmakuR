...
namespace DanmakuR.Protocol.Buffer.Writers;
...
#pragma warning restore CS8774 // Member must have a non-null value when exiting.
	...
	public override void Write(byte[] buffer, int offset, int count)
	{
		...
		{
			System.Buffer.BlockCopy(buffer, offset, _currentSegment, position, count);
	...
	public WrittenSequence DeatchToSequence()
	{
		_completedSegments ??= new List<CompletedBuffer>();
		...
		var seq = new WrittenSequence(_completedSegments, _bytesWritten);
	...
	/// You are now responsible for calling Dispose on this type to return the memory to the pool.
	/// </summary>
	internal readonly struct WrittenSequence : IDisposable
	{
		private sealed class Segment : ReadOnlySequenceSegment<byte>
		{
			...
			public Segment(byte[] buffer, int end)
			{
				_buffer = buffer;
				_end = end;

				Memory = buffer.AsMemory(0, end);
			}
			...
			public Segment CreateNext(byte[] buffer, int end)
			{
				NextSegment = new(buffer, end);
